...
class mBERT(torch.nn.Module):
    def __init__(self):
        ...
        self.linear_level1 = nn.Sequential(nn.Linear(768, 1024), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(1024, 512), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(512, 128), nn.ReLU())

        self.linear_level2 = nn.Sequential(nn.Linear(768, 1024), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(1024, 512), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(512, 128), nn.ReLU())
        self.linear_level3 = nn.Sequential(nn.Linear(768, 1024), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(1024, 512), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(512, 128), nn.ReLU())
        self.linear_level4 = nn.Sequential(nn.Linear(768, 1024), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(1024, 512), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(512, 128), nn.ReLU())
        self.linear_level5 = nn.Sequential(nn.Linear(768, 1024), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(1024, 512), nn.ReLU(), nn.Dropout(dropout_rate),
                                           nn.Linear(512, 128), nn.ReLU())
